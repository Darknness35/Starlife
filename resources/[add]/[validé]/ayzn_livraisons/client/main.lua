...
--[[ 	if PlayerJob ~= "delivery" then
	...
	if CurrentStatus ~= Status.DELIVERY_INACTIVE then
		...
		DrawMarker(20, deleter.x, deleter.y, deleter.z, 0, 0, 0, 0, 180.0, 0, 1.5, 1.5, 1.5, 249, 38, 114, 100, false, true, 2, false, false, false, false)
		...
		if CurrentStatus == Status.PLAYER_STARTED_DELIVERY then
			if not IsPlayerInsideDeliveryVehicle() and CurrentVehicle ~= nil then
				...
				DrawMarker(20, VehiclePos.x, VehiclePos.y, ArrowHeight, 0, 0, 0, 0, 180.0, 0, 0.8, 0.8, 0.8, 102, 217, 239, 100, false, true, 2, false, false, false, false)
			else
				local dl = DeliveryLocation.Item1
				if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, dl.x, dl.y, dl.z, true) < 150 then
					DrawMarker(20, dl.x, dl.y, dl.z, 0, 0, 0, 0, 180.0, 0, 1.5, 1.5, 1.5, 102, 217, 239, 100, false, true, 2, false, false, false, false)
				end
			end
		end
		...
		if CurrentStatus == Status.PLAYER_REACHED_VEHICLE_POINT then
			if not IsPlayerInsideDeliveryVehicle() then
				...
				DrawMarker(20, TrunkPos.x, TrunkPos.y, TrunkHeight, 0, 0, 0, 180.0, 0, 0, ArrowSize.x, ArrowSize.y, ArrowSize.z, 102, 217, 239, 100, false, true, 2, false, false, false, false)
		...
		if CurrentStatus == Status.PLAYER_REMOVED_GOODS_FROM_VEHICLE then
			...
			DrawMarker(20, dp.x, dp.y, dp.z, 0, 0, 0, 0, 180.0, 0, 1.5, 1.5, 1.5, 102, 217, 239, 100, false, true, 2, false, false, false, false)
		end
		...
		if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, bCoords.x, bCoords.y, bCoords.z, true) < 150.0 then
			...
			DrawMarker(37, ScooterPos.x, ScooterPos.y, ScooterPos.z, 0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 243, 56, 56, 100, false, true, 2, false, false, false, false)
			DrawMarker(36, VanPos.x, VanPos.y, VanPos.z, 0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 250, 170, 60, 100, false, true, 2, false, false, false, false)
			DrawMarker(39, TruckPos.x, TruckPos.y, TruckPos.z, 0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 230, 219, 91, 100, false, true, 2, false, false, false, false)
			...
			if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, ScooterPos.x, ScooterPos.y, ScooterPos.z, true) < 1.5 then
				DisplayHelpText(Config.Locales["start_delivery"])
				SelectType = 'scooter'
