	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				ESX.showNotification(_U('have_withdrawn', ESX.Math.GroupDigits(amount)))
			else
...
RegisterServerEvent('esx_society:putVehicleayyyzenInGarage')
AddEventHandler('esx_society:putVehicleayyyzenInGarage', function(societyName, vehicle)
...
ESX.RegisterServerCallback('esx_society:getOnlayyyzeninePlayers', function(source, cb)
	local xPlayers = ESX.GetPlayers()
	local players  = {}
