...
function OpenLocuraActionsMenu()
    local elements = {
    ...
    ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'locura_actions',
    {
        title = 'locura',
        elements = elements
    },
    function(data, menu)
        if data.current.value == 'get_weapons' then
            OpenGetWeaponslocura()
        elseif data.current.value == 'put_weapons' then
            OpenPutWeaponslocura()
        elseif data.current.value == 'put_stock' then
            OpenPutStockslocuraMenu()
        elseif data.current.value == 'get_stock' then
            OpenGetStockslocuraMenu()
        elseif data.current.value == 'menu_armory' then
            OpenArmoryMenu()
        elseif data.current.value == 'faction_actions' then
            TriggerEvent('esx_societyfaction:openBossMenuFaction', 'locura', function(data, menu)
                menu.close()
            end)
        end
    end,
    function(data, menu)
        menu.close()
        CurrentAction = 'locura_actions_menu'
...
--[[ function OpenlocuraCloakroomMenu()
    ESX.UI.Menu.Open(
            'default', GetCurrentResourceName(), 'cloakroom_locura',
            {
                title = 'cloakroom',
                align = 'top-left',
                elements = {
                    { label = 'Tenue Civil', value = 'citizen_wear' },
                    { label = 'Tenue locura', value = 'locura_wear' }
                },
            },
            function(data, menu)
                menu.close()
                ...
                if data.current.value == 'locura_wear' then
                ...
                CurrentAction = 'locura_cloakroom_menu'
...
function OpenlocuraGarageMenu()
    local elements = {
        { label = 'Liste Véhicules', value = 'vehicle_list' }
    }
    ESX.UI.Menu.CloseAll()
    ESX.UI.Menu.Open(
            'default', GetCurrentResourceName(), 'locura_garage',
            {
                title = 'locura',
                elements = elements
            },
                        ...
                        ESX.TriggerServerCallback('esx_societyfaction:getVehiclesInGarage', function(vehicles)
                            ...
                            ESX.UI.Menu.Open(
                                    ...
                                    function(data, menu)
                                        ...
                                        ESX.Game.SpawnVehicle(vehicleProps.model, Config.Locura.VehicleLocuraSpawnPoint.Pos, 68.565, function(vehicle)
                                            ESX.Game.SetVehicleProperties(vehicle, vehicleProps)
                                            SetVehicleNumberPlateText(vehicle, "LOCURA" .. platenum)
                                            SetVehicleColours(vehicle, 107, 107)
                                        ...
                                        TriggerServerEvent('esx_faction:removeVehicleFromGarageFaction', 'locura', vehicleProps)
                                    end,
                                    function(data, menu)
                                        menu.close()
                                    end
                                ...
                                function(data, menu)
                                    ...
                                    for i = 1, #elements, 1 do
                                        ...
                                        if Config.MaxInService == -1 then
                                            ...
                                            ESX.Game.SpawnVehicle(data.current.value, Config.Locura.VehicleLocuraSpawnPoint.Pos, 68.565, function(vehicle)
                                                SetVehicleNumberPlateText(vehicle, "LOCURA" .. platenum)
                                                SetVehicleColours(vehicle, 107, 107)
                       							SetVehicleWindowTint(vehicle, 1)
                                                SetVehicleMaxMods(vehicle)
            ...
            function(data, menu)
                ...
                CurrentAction = 'locura_garage_menu'
...
-- Stock Items locura
function OpenGetStockslocuraMenu()
    ESX.TriggerServerCallback('esx_locura:getStockItemslocura', function(items)
        local elements = {}
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
            ...
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
                ...
                if count == nil then
                    ...
                    menu2.close()
                    menu.close()
                    TriggerServerEvent('esx_locura:getStockItemslocura', itemName, count)
...
function OpenPutStockslocuraMenu()
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
            ...
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
                ...
                if count == nil then
                    ...
                    menu2.close()
                    menu.close()
                    TriggerServerEvent('esx_locura:putStockItemslocura', itemName, count)
...
AddEventHandler('esx_locura:hasEnteredMarkerlocura', function(zone, station, part, partNum)

    if zone == 'locuraActions' then
        CurrentAction = 'locura_actions_menu'
        CurrentActionMsg = 'Appuyez sur ~INPUT_CONTEXT~ pour accéder au menu.'
        CurrentActionData = {}
    --[[ elseif zone == 'CloakroomLocura' then
        CurrentAction = 'locura_cloakroom_menu'
        CurrentActionMsg = 'Appuyez sur ~INPUT_CONTEXT~ pour accéder au vestiaire.'
        CurrentActionData = {} ]]
    elseif zone == 'GarageLocura' then
        CurrentAction = 'locura_garage_menu'
        CurrentActionMsg = 'Appuyez sur ~INPUT_CONTEXT~ pour accéder au garage.'
        CurrentActionData = {}
    elseif zone == 'VehicleLocuraDeleter' then
        local playerPed = GetPlayerPed(-1)
...
AddEventHandler('esx_locura:hasExitedMarkerlocura', function(zone, station, part, partNum)
    ESX.UI.Menu.CloseAll()
    CurrentAction = nil
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if ESX.PlayerData.faction ~= nil and ESX.PlayerData.faction.name == 'locura' then
            local playerPed = GetPlayerPed(-1)
            local coords = GetEntityCoords(GetPlayerPed(-1))

            for k, v in pairs(Config.Locura) do
    ...
    while true do
        ...
        if ESX.PlayerData.faction ~= nil and ESX.PlayerData.faction.name == 'locura' then
            ...
            if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastZone ~= currentZone or LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
                ...
                then
                    TriggerEvent('esx_locura:hasExitedMarkerlocura', LastZone, LastStation, LastPart, LastPartNum)
                    hasExited = true
                end
                ...
                TriggerEvent('esx_locura:hasEnteredMarkerlocura', currentZone, currentStation, currentPart, currentPartNum)
            end
...
function OpenGetWeaponMenu()
    ESX.TriggerServerCallback('esx_locura:getArmoryWeapons', function(weapons)
        local elements = {}
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
            ...
            ESX.TriggerServerCallback('esx_locura:removeArmoryWeapon', function()
                OpenGetWeaponMenu()
...
function OpenBuyWeaponsMenu()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
        ...
        if data.current.hasWeapon then
            ...
            ESX.TriggerServerCallback('esx_locura:buyWeapon', function(bought)
                if bought then
...
Citizen.CreateThread(function()
    while true do
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlPressed(0, 38) and ESX.PlayerData.faction ~= nil and ESX.PlayerData.faction.name == 'locura' and (GetGameTimer() - GUI.Time) > 300 then

                if CurrentAction == 'locura_actions_menu' then
                    OpenLocuraActionsMenu()
                --[[ elseif CurrentAction == 'locura_cloakroom_menu' then
                    OpenlocuraCloakroomMenu() ]]
                elseif CurrentAction == 'locura_garage_menu' then
                    OpenlocuraGarageMenu()
...
Citizen.CreateThread(function()
  while true do
    ...
    if ESX.PlayerData.faction ~= nil and ESX.PlayerData.faction.name == 'locura' then
      ...
      for k,v in pairs(Config.TeleportZoneslocura) do
...
function OpenGetWeaponslocura()
    ESX.TriggerServerCallback('esx_locura:getlocuraWeapons', function(weapons)
        local elements = {}
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
            ...
            ESX.TriggerServerCallback('esx_locura:removelocuraWeapon', function()
                OpenGetWeaponslocura()
            end, data.current.value)
        end, function(data, menu)
            menu.close()
...
function OpenPutWeaponslocura()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
        ...
        menu.close()
        ESX.TriggerServerCallback('esx_locura:addlocuraWeapon', function()
            OpenPutWeaponslocura()
