...
AddEventHandler('playerConnecting', function(name, reject, def)
	...
	def.presentCard([==[{
		...
		{
		    "type":"TextBlock",
		    "text":"Rendez vous sur le discord pour vous connecter !",
		    "wrap":true
		},
		{
		    "type":"ActionSet",
		...
		--else
			--def.done('Mauvais mot de passe, il est trouvable sur le discord : discord.gg/5yW9EPD')
		--end
	end)
end)
...
RegisterServerEvent("starlife-rocade:playerKicked")
AddEventHandler("starlife-rocade:playerKicked", function(src, points)
...
RegisterServerEvent("starlife-rocade:playerConnected")
AddEventHandler("starlife-rocade:playerConnected", function()
