	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'put_stock_tabac' then
    			OpenPutStocksTabacMenu()
			end	

			if data.current.value == 'get_stock_tabac' then
				OpenGetStocksTabacMenu()
			end	
...
function OpenTabacCraft2Menu()
	...
	local elements = {
		{label = 'Assembler Malbora', value = 'craft_malbora'},
...
function OpenGetStocksTabacMenu()

	ESX.TriggerServerCallback('nwx_tabac:getStockItemsTabac', function(items)
	  ...
	  ESX.UI.Menu.Open(
	    'default', GetCurrentResourceName(), 'stocks_tabac_menu',
	    {
	    	css = 'entreprise',
		    title = 'Malborose Stock',
		    elements = elements
	    },
	    function(data, menu)
	    	...
	    	local itemName = data.current.value
				...
				ESX.UI.Menu.Open(
					'dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count_tabac',
					{
						title = 'Quantité'
					},
					function(data2, menu2)
						...
						menu2.close()
				    	menu.close()
							TriggerServerEvent('nwx_tabac:getStockItemTabac', itemName, count)
							Citizen.Wait(1000)
							OpenGetStocksTabacMenu()
						end
...
function OpenPutStocksTabacMenu()
	...
	ESX.TriggerServerCallback('nwx_jobs:getPlayerInventory', function(inventory)
			...
			ESX.UI.Menu.Open(
				'default', GetCurrentResourceName(), 'stocks_menu_tabac',
				{
					css = 'inventaire',
					title = 'Inventaire',
					 elements = elements
				...
				function(data, menu)
					...
					ESX.UI.Menu.Open(
						'dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count_tabac',
						{
							title = 'Quantité'
						},
						function(data2, menu2)
							...
							TriggerServerEvent('nwx_tabac:putStockItemsTabac', itemName, count)
							Citizen.Wait(1000)
							OpenPutStocksTabacMenu()
							end
