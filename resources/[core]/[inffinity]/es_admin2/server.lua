				...
				if(GetPlayerName(USER) == nil)then
					...
					TriggerEvent("es:getAllGroups", function(groups)
						...
						if(groups[GROUP])then
							...
							TriggerEvent("zey:blacklisted", USER, "group", GROUP, function(response, success)
			...
			elseif t == "level" then
				...
				if(GetPlayerName(USER) == nil)then
					...
					if(GROUP ~= nil and GROUP > -1)then
						...
						TriggerEvent("zey:blacklisted", USER, "permission_level", GROUP, function(response, success)
...
AddEventHandler('rconCommand', function(commandName, args)
	...
	if commandName == 'setadmin' then
		...
		TriggerEvent("zey:blacklisted", tonumber(args[1]), "permission_level", tonumber(args[2]), function(response, success)
	...
	elseif commandName == 'setgroup' then
		...
		TriggerEvent("es:getAllGroups", function(groups)
			...
			if(groups[args[2]])then
				...
				TriggerEvent("zey:blacklisted", tonumber(args[1]), "group", args[2], function(response, success)
...
--[[ TriggerEvent('es:addGroupCommand', 'ghost', "mod", function(source, args, user)
...
end, {help = "Enable or disable ghostmode"})



--god

TriggerEvent('es:addGroupCommand', 'god', "mod", function(source, args, user)
